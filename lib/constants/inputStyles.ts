export const INPUT_STYLES = {
  base: 'w-full px-4 py-3 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 transition-colors',
  focus:
    'focus:outline-none focus:ring-2 focus:ring-inset focus:ring-brand-500 focus:border-transparent focus:bg-white dark:focus:bg-neutral-900',
  error: 'border-red-500 focus:ring-red-200 dark:focus:ring-red-900',
  disabled: 'opacity-50 cursor-not-allowed',
  placeholder: 'placeholder:text-neutral-400 dark:placeholder:text-neutral-500',
};

export const INPUT_SIZE = {
  sm: 'px-3 py-2 text-sm',
  md: 'px-4 py-3 text-base',
  lg: 'px-5 py-4 text-lg',
};

export const FORM_ERROR_STYLES = {
  container: 'mt-1 text-sm text-red-600 dark:text-red-400',
  input: 'border-red-500 focus:border-red-500 focus:ring-red-200 dark:focus:ring-red-900',
};

export const LABEL_STYLES = {
  base: 'block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2',
};
